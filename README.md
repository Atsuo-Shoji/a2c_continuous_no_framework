...
共有型が機能しなかったのは、「Pendulumの仕様により莫大なCriticのlossが半永続的に発生し続けたから」からだと思います。<br>
...
一方、Actorのlossはマイナス数百くらいです。<br>
従って、**CriticのlossとActorのlossに著しい不均衡が生じ続け、共有部分のパラメーター調整がうまくいかなかった**のだと思います。<br>

ちなみに、共有型のモデル全体のlossの計算式<br>
全体の損失 = Actorの損失 + 重み係数 * Criticの損失<br>
の重み係数を0.1などにしてみましたが、効果ありませんでした。文字通り「ケタが違う」のだと思います。<br>
これ以上重み係数を小さくすると、価値関数の最適化の方が果たせなくなると思ったので、これ以上小さくしませんでした。<br>
...
それでもPendulumの時のように共有型が崩壊しなかったのは、1エピソードのステップ数が（Pendulumと異なり）固定ではないから、と思います。<br>

Agentの行動次第でステップ数に変化がつくので、訓練初期は毎エピソード「必ず」累積報酬和が大きなマイナスの数値になるとは限らず、累積報酬和が大きなマイナスの数値になる「傾向」がある、としかならないです。<br>
よって、訓練初期であっても共有型においてNN共有部分が丸かぶりするCriticのlossは「必ず」莫大な数値になるわけではなく、そうなる「傾向」があるだけ、となります。<br>
共有型のlossのグラフはまさにそうなっています。<br>
ここがPendulumとの違いとなります。<br>

ただこの<b>「NN共有部分が丸かぶりするCriticのlossは莫大な数値になる”傾向”がある」は分離型に対してハンデ</b>であることは明らかで、共有型は分離型より劣ってしまうのだと思います。<br>
...
これにより、ほぼ同一の入力stateであってもエピソード終端までの割引報酬和が大きく異なる⇒割引報酬和の分散が大きくなる、ということになります。<br>
従って、Critic側の損失関数の教師信号（＝割引報酬和）、共有型の場合はActorの損失関数に与えるadvantage（＝割引報酬和－価値関数）の分散も大きくなる、ということになります。<br>

このように訓練が不安定になり、結果、進まなくなる、ということになると思います。<br>
    
③割引報酬和の絶対値が大きすぎ<br>

Pendulumの10数万は言うに及ばず、他の環境でも（割引報酬和と価値関数Vの差分の2乗である）Criticのlossは、Actorのlossと大きくバランスを欠く大きさでした。<br>
共有型では、このアンバランスはNNのほぼ全体に影響を与えてしまいます。同時に、Advantageを介して、Actorのlossそのものを大きく変動させてしまいます。<BR>
分離型でも、同じくAdvantageを介してActorのlossそのものを大きく変動させてしまう問題自体は残ります。<br>
分離型もあまり良くありませんでしたが、共有型の方が分離型よりさらにパフォーマンスが悪かった、という結果はこのせいかもしれません。<br>
**一般の実装例において、割引報酬和を標準化しているものが散見されるので、そのように修正**しようと思います。<br>
...
- 共有部分の表現力不足<br>
共有型の中間層である共有部分は、「方策（正確にはμとlog(var)）」と「価値関数」を出力するための主要部分となっています。<br>
つまり、**共有型の共有部分は、「方策」と「価値関数」双方に共通して通用する”汎用的な”関数である必要**があります。<br>
一方で、分離型のActorの中間層は「方策」だけを、Criticの中間層は「価値関数」だけを出力するための主要部分となります。<br>
従って、共有型の共有部分の構成は、比較のため、分離型のActorとCriticのNNの中間層と同じ構成にしていることも鑑みて、<BR>
**共有型の中間層（共有部分）は、分離型の各々の中間層よりも重い役目を背負わされているにもかかわらず、それら分離型の各々の中間層と同じ表現力しか与えられていない**、ということになります。<br>
