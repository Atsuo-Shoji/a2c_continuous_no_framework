...
|HopperPyBullet|![hopper_mini](https://user-images.githubusercontent.com/52105933/95576384-50e69800-0a6b-11eb-9a18-711c04d690c9.png)|15|3|1000|遠くまで歩いて（跳ねて？）前進|
...
※ただし、Actorの損失をAdvantageを介してCritic側に逆伝播させるのは、必ずしも間違いではないようです。<BR>
モデル全体のloss（= Actorのloss + 重み係数 * Criticのloss）の最小化の実現可能性を主要な視点とします。<BR>
解きたいタスクの複雑さによって、この実現可能性が決まります。<BR>
純理論的には、モデル全体のlossを最小化するには、順伝播と同じルートの逆伝播が最も実現可能性が高いに決まっています。<BR>
本モデルでは、入力値Stateが共通部分を通って来て、（わずか1layerの）Actorの出力、Criticの出力を経由し、それぞれの損失となります。<BR>
この構造の元で、Actorのloss値、Criticのloss値がそれぞれ「独立に」妥当であれば（妥当であるくらいタスクがシンプルであれば）、Actorの損失をAdvantageを介してCritic側に逆伝播させなくても、モデル全体のlossを容易に最小化できるでしょう。<BR>
しかし、妥当でなければ、Actorのlossを最小化する手続きにおいてCriticの出力Vを直接変動させるようにしないと、そもそもモデル全体のlossの最小化の実現可能性が乏しくなります。
...
状態24次元＆行動4次元という高次元で、且つ2000ステップという長大なエピソードだと、エピソード毎の状態と行動のバリエーションがありすぎて、どのような（その長い一連の）行動を取ればそのような報酬がもたらされるのか、2.5万回程度のイテレーションではイマイチ学習し切れないのかもしれません。<br>
**＜追記＞**<br>
BipedalWalkerのオリジナルの報酬設計が原因、と強く推測しています。<br>
BipedalWalkerは、失敗時（転倒時）、-100という「桁違いに大きい絶対値の」即時報酬を返してエピソードを終了します。<br>
通常のステップに対する即時報酬は、この-100に比べ著しく数値規模が小さいです。<br>
これでは訓練が進まないはずで、折を見て、例えば「失敗時（転倒時）は-1、成功時は+1，それ以外はオリジナルの即時報酬のまま」といった報酬設計に変更し、再訓練しようと思います。
